...
class ColorController:
    @staticmethod
    def get_palette_accessibility_audit_control(
        request: PaletteAccessibilityAuditRequest,
    ) -> PaletteAccessibilityAuditResponse:
        try:
            return ColorService.get_palette_accessibility_audit(request)
    ...
    @staticmethod
    def generate_palette_from_image_control(image_bytes: bytes, count: int) -> PaletteGenerateResponse:
        try:
            return PaletteImageService.extract_dominant_palette(image_bytes, count)
        except ValueError as e:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
        except Exception as e:
            raise HTTPException(
